...
void array_insert_back(Array* a, long stuff)
{
    if (a->capacity < (a->back + 2))
    {
        //probably some more optimal way to decide re-allocation-space, but this is good enough
...
void ovf_array_insert_back(Array* a, long stuff)
{
    a->data[a->back] = stuff;
    a->back++;
}